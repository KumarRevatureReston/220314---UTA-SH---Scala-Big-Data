...
CREATE TABLE IF NOT EXISTS countries(country_id int DEFAULT NULL, country_name varchar(20) DEFAULT NULL, region_id int DEFAULT NULL);
...
CREATE TABLE IF NOT EXISTS countries(country_id int, country_name varchar(20) CHECK(country_name in ('Italy', 'India', 'China')), region_id int);
...
CREATE TABLE IF NOT EXISTS job_history(employee_id int, start_date date, end_date date CHECK(end_date LIKE '--/--/----'), job_id int, department_id int);
...
CREATE TABLE IF NOT EXISTS countries(country_id int, country_name varchar(20), region_id int, UNIQUE(country_id));

-- 10. Write a SQL statement to create a table named jobs including columns job_id, job_title, min_salary and max_salary, 
-- and make sure that, the default value for job_title is blank and min_salary is 8000 and max_salary is NULL will be entered automatically at the time of insertion if no value assigned for the specified columns.
CREATE TABLE IF NOT EXISTS jobs(job_id int, job_title varchar(20) DEFAULT ' ', min_salary int DEFAULT 8000, max_salary int DEFAULT NULL);
...
CREATE TABLE IF NOT EXISTS countries(country_id int NOT NULL PRIMARY KEY, country_name varchar(20), region_id int, UNIQUE(country_id));
...
CREATE TABLE IF NOT EXISTS countries(country_id int NOT NULL AUTO_INCREMENT, country_name varchar(20), region_id int, UNIQUE(country_id));
...
CREATE TABLE IF NOT EXISTS countries(country_id int, country_name varchar(20), region_id int, UNIQUE(country_id), UNIQUE(region_id));
...
-- and make sure that, the employee_id column does not contain any duplicate value at the time of insertion and the foreign key column job_id contain only those values 
-- which are exists in the jobs table.
CREATE TABLE IF NOT EXISTS job_history(employee_id int, start_date date, end_date date, job_id int, department_id int, UNIQUE(employee_id), FOREIGN KEY(job_id) REFERENCES jobs(job_id));
...
CREATE TABLE IF NOT EXISTS employees(employee_id int UNIQUE, first_name varchar(20), last_name varchar(20), email varchar(40), phone_number int, hire_date date, job_id int, salary int, commission int, manager_id int, department_id int, FOREIGN KEY(department_id, manager_id) REFERENCES departments(department_id, manager_id));
...
-- which are exists in the departments table and another foreign key column job_id, referenced by the column job_id of jobs table, can contain only those values 
...
-- The ON DELETE CASCADE that lets you allow to delete records in the employees(child) table that refer to a record in the jobs(parent) table when the record in 
...
-- 19. Write a SQL statement to create a table employees including columns employee_id, first_name, last_name, job_id, salary and make sure that, the employee_id column 
-- does not contain any duplicate value at the time of insertion, and the foreign key column job_id, referenced by the column job_id of jobs table, 
-- can contain only those values which are exists in the jobs table. The InnoDB Engine have been used to create the tables. The specialty of the statement is that, 
-- The ON DELETE SET NULL action will set the foreign key column values in the child table(employees) to NULL when the record in the parent table(jobs) is deleted, 
-- with a condition that the foreign key column in the child table must accept NULL values and the ON UPDATE SET NULL action resets the values in the rows in the 
-- child table(employees) to NULL values when the rows in the parent table(jobs) are updated.
...
VALUES
('US', 'United States of America', 01);
...
INSERT INTO counties (COUNTRY_ID, COUNTRY_NAME) 
VALUES
...
-- 3. Write a SQL statement to change the email column of employees table with 'not available' for those employees whose department_id is 80 and 
-- gets a commission is less than .20%
UPDATE employees SET email = 'not available', WHERE department_id = 80 AND comission_pct < 0.20;
-- 4.Write a SQL statement to increase the salary of employees under the department 40, 90 and 110 according to the company rules that, salary will be 
