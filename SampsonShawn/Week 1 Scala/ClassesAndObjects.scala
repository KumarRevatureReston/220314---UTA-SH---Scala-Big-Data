...
/*In Scala, we can also create a primary constructor with a default value. If you don’t provide value,
then it will take the default value, which is provided in the primary constructor.  Otherwise, the value
  we provide is from an instance created with the help of a parameter name of the class. Here is an example:*/

class Employee2(empId: Int = 0,name: String = "",salary :Double = 0.0){
  println(s"empId = $empId , empName = $name , salary = $salary")
}
object Employee2Obj extends App {
  val emp = new Employee2()
  //empId = 0, empName =, salary = 0.0

  val emp1 = new Employee2(name = "Smith")
  //empId = 0 , empName = Smith , salary = 0.0


  val emp2 = new Employee2(name = "Smith", empId = 9)
...
object Reservations extends App {
  var lastNum = 0;
  val num2 = 2;
...
object GFG extends App {
  ...
  def notMain(args:Array[String]) = {
    try { var N = 5/0
      ...
      case i: IOException => { println("IOException occurred.") }
...
// Creating object
object GFG2 extends App {
  // Main method
  def notMain(args: Array[String]) = {
    try { var N = 5/0 }
    catch{
      // Catch block contain case.
      case ex: ArithmeticException => { println("Arithmetic Exception occurred.") }
    }
    finally {
...
object PatternMatchingAndRegularExpressions extends App {
