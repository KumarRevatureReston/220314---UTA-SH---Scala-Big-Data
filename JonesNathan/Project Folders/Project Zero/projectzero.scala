...
object CSV{
  def readCSV(file: String): Seq[String] = {
    val bufferedSource = io.Source.fromFile("C:/Users/natha/databases/airports4.csv")   
...
object projectZero {
  def main(args: Array[String]): Unit = {
        ...
        val conn = DriverManager.getConnection(conn_url, "postgres", "password") //set password 
        println("")
...
object createTable {
    def column(conn: Connection, s: Seq[String] ) = {
        ...
        while(inc < s.length-1){           //increment to run INSERT over sets of data
          ...
          var record = ""
          if (rec.length <= 18) {     //know that it is 18 columns
            for (n <- rec){                 
...
object createRecord{
  def ins(args: Array[String]){
    val conn_url = "jdbc:postgresql://localhost:5432/uni_students"
    val conn = DriverManager.getConnection(conn_url, "postgres", "password")    //insert password
    val stm = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)
...
object readRecord{
  def read(args: Array[String]){
    ...
    var userinput = scala.io.StdIn.readLine()
    println("Provide conditional statement if desired. Ex: WHERE column = 'value'")
    var userinput2 = scala.io.StdIn.readLine()
